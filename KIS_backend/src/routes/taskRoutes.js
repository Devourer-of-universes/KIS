...
// Получение всех задач пользователя (с фильтрацией)
router.get('/tasks', authMiddleware, async (req, res) => {
    try {
        const userId = req.userId;
        const { status, priority, startDate, endDate, assignedTo } = req.query;
...
router.get('/tasks/:id', authMiddleware, async (req, res) => {
    try {
        ...
        const result = await query(`
            SELECT t.*, 
                   u.surname as assignee_surname, u.name as assignee_name, u.avatar_uri as assignee_avatar,
                   c.surname as creator_surname, c.name as creator_name,
                   d.name as department_name,
                   (SELECT json_agg(json_build_object('id', tc.id, 'content', tc.content, 'created_at', tc.created_at, 'user_id', tc.user_id, 'user_name', u2.surname || ' ' || u2.name))
            ...
            WHERE t.id = $1 AND t.deleted_at IS NULL
              AND (t.created_by = $2 OR t.assigned_to = $2 OR $2 IN (SELECT user_id FROM users WHERE is_super_admin = true))
        `, [id, userId]);
...
router.post('/tasks', authMiddleware, async (req, res) => {
    try {
        const { title, description, assignedTo, departmentId, startDate, dueDate, priority, tags } = req.body;
        const userId = req.userId;
        ...
        const result = await query(`
            INSERT INTO tasks (title, description, created_by, assigned_to, department_id, start_date, due_date, priority, tags, status, progress)
            VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, 'pending', 0)
            RETURNING *
        `, [title.trim(), description, userId, assignedTo || null, departmentId || null, startDate || null, dueDate || null, priority || 'medium', tags || []]);
        
        // Логируем создание
        await query(`
            INSERT INTO task_history (task_id, user_id, field_name, new_value)
            VALUES ($1, $2, 'created', 'Задача создана')
...
router.put('/tasks/:id', authMiddleware, async (req, res) => {
    try {
        ...
        if (status === 'completed' && oldTask.rows[0].status !== 'completed') {
        ...
        const changes = [];
        if (old.title !== newTask.title) changes.push({ field: 'title', old: old.title, new: newTask.title });
        if (old.assigned_to !== newTask.assigned_to) changes.push({ field: 'assigned_to', old: old.assigned_to, new: newTask.assigned_to });
        if (old.due_date !== newTask.due_date) changes.push({ field: 'due_date', old: old.due_date, new: newTask.due_date });
        if (old.status !== newTask.status) changes.push({ field: 'status', old: old.status, new: newTask.status });
        
        for (const change of changes) {
            await query(`
                INSERT INTO task_history (task_id, user_id, field_name, old_value, new_value)
                VALUES ($1, $2, $3, $4, $5)
            `, [id, userId, change.field, String(change.old), String(change.new)]);
        }
...
router.delete('/tasks/:id', authMiddleware, async (req, res) => {
    try {
        ...
        const result = await query(`
            UPDATE tasks SET deleted_at = CURRENT_TIMESTAMP WHERE id = $1 AND created_by = $2 RETURNING id
        `, [id, userId]);
        
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'Задача не найдена или нет прав' });
        }
...
// Получение событий для календаря (задачи + дедлайны)
